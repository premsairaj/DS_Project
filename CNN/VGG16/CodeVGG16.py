# -*- coding: utf-8 -*-
"""
Created on Tue Jun 14 22:16:32 2022
...
def augment(img1):
  ...
  #rotation
    ...
    img1 = cv2.rotate(img, cv2.cv2.ROTATE_90_COUNTERCLOCKWISE)#Rotating the Image Anti-Clockwise direction
    img1=cv2.resize(img1,(300,300))
    img2 = cv2.rotate(img, cv2.cv2.ROTATE_90_CLOCKWISE)#Rotating the Image Clockwise direction
...
x_valid=[]#Exracting Validation Samples
y_valid=[]#Extracting Validation Sample Labels
...
x_train=np.array(x_train).astype('float32')/255.0#converting into 4 dim-tensor 
...
y_train = np_utils.to_categorical(y_train, 3)#converting into Label Encoding format
y_valid= np_utils.to_categorical(y_valid, 3)

vgg16=VGG16(include_top=False,classes=3,input_shape=(300,300,3))#clone the VGG16 Model
for layers in vgg16.layers:#trainable parameters
    layers.trainable=False
vgg16.summary()

x=Flatten()(vgg16.output)
Preduction=Dense(3,activation='softmax',)(x)
model=Model(inputs=vgg16.input,outputs=Preduction)
